...
const Status createHeapFile(const string fileName)
{
    ...
    if (status != OK)
    {
        ...
        Page *hdrPageTmp;
        if ((status = bufMgr->allocPage(file, hdrPageNo, hdrPageTmp)) != OK)
        {
            return status;
        }

        // cast to FileHdrPage
        hdrPage = (FileHdrPage *)hdrPageTmp;

        // allocate empty data page
...
HeapFile::HeapFile(const string &fileName, Status &returnStatus)
{
    ...
    if ((status = db.openFile(fileName, filePtr)) == OK)
    {
        ...
        {
            cerr << "failed to get pageNo" << endl;
            returnStatus = status;
            return;
        }
        // read first page
        ...
        {
            cerr << "failed to read header page" << endl;
            returnStatus = status;
            return;
        }
        ASSERT(pageNo != -1);

        // cast first page to FileHdrPage and update stats
        headerPage = (FileHdrPage *)pagePtr;
        hdrDirtyFlag = false;
        headerPageNo = pageNo;

        int firstPageNo = curPageNo = headerPage->firstPage;
        if ((status = bufMgr->readPage(filePtr, firstPageNo, pagePtr)) != OK)
        {
            cerr << "failed to read data page";
            returnStatus = status;
            return;
        }

        // update curPage stats
        curPage = pagePtr;
        curDirtyFlag = false;
        returnStatus = OK;
        curRec = NULLRID;
...
const Status HeapFile::getRecord(const RID &rid, Record &rec)
{
    ...
    {
        // curPage is NULL or pinned but incorrect
